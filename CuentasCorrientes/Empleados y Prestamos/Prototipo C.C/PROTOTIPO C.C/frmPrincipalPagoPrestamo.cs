...
namespace PROTOTIPO_C.C
{
    public partial class frmPrincipalPagoPrestamo : Form
    {
        ...
        void funActualizarGrid()
        {
            ...
            cnegocio.funconsultarRegistros("prestamo", "SELECT prestamo.codPrestamo AS CODIGO, personas.dpi AS DPI, CONCAT(empleado.cod_empleado, ' ', personas.nombres,' ', personas.apellidos) AS NOMBRE , prestamo.fecha AS FECHA, prestamo.interes AS INTERES, prestamo.pagos AS PAGOS, prestamo.total as TOTAL, prestamo.Saldo AS SALDO FROM prestamo, empleado, personas WHERE empleado.codigo_persona=personas.codigo_persona AND prestamo.cod_empleado=empleado.cod_empleado AND prestamo.estado='ACTIVO' AND prestamo.condicion='ACTIVO' AND prestamo.saldo<prestamo.total", "consulta", grdPrestamos);
        }
        ...
        private void grdPrestamos_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            string sSaldo = grdPrestamos.Rows[grdPrestamos.CurrentCell.RowIndex].Cells[7].Value.ToString();
            frmPagoPrestamos m = new frmPagoPrestamos(sCodPrestamo, sSaldo);
            m.ShowDialog();
            funActualizarGrid();
